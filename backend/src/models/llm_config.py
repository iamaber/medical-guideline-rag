...
No need for provider-specific keys anymore.
"""
...
class LLMSettings(BaseSettings):
    ...
    Simplified configuration - single API key for all providers.
    ...
    # Unified API Key (works for all providers)
    llm_api_key: Optional[SecretStr] = Field(
        default=None, description="API key for the selected LLM provider"
    )

    # Backward compatibility: legacy provider-specific keys (for migration)
    ...
    zai_api_key: Optional[SecretStr] = Field(default=None)
    ...
    def get_api_key_for_provider(
        ...
        Priority order:
        1. Unified LLM_API_KEY (recommended)
        2. Provider-specific key (backward compatibility)

        Args:
            provider: The provider to get API key for. If None, uses the
        ...
        # First, check unified API key
        if self.llm_api_key:
            return self.llm_api_key.get_secret_value()

        # Fallback to provider-specific keys for backward compatibility
        key_map = {
