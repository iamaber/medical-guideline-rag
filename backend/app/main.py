...
async def health_check():
    ...
    if llm_client:
        services_status["llm_client"] = llm_client.is_available
...
async def get_medication_advice(user_input: UserInput):
    ...
    try:
        ...
        if not all(
            [drug_lookup, jina_scraper, vector_search, llm_client, knowledge_graph]
        ):
            raise HTTPException(
                status_code=503,
...
async def get_system_stats():
    ...
    try:
        ...
        if llm_client:
            stats["services"]["llm_client"] = llm_client.get_model_info()
