...
class Settings(BaseSettings):
    ...
    def effective_google_api_key(self) -> Optional[str]:
        """Get Google API key with backwards compatibility for GEMINI_API_KEY."""
    ...
    @property
    def effective_jina_api_key(self) -> Optional[str]:
        """Get Jina API key as string."""
        if self.jina_api_key:
            return self.jina_api_key.get_secret_value()
        return None

    @property
    def effective_ncbi_api_key(self) -> Optional[str]:
        """Get NCBI API key as string."""
        if self.ncbi_api_key:
            return self.ncbi_api_key.get_secret_value()
        return None

    def get_llm_settings(self) -> LLMSettings:
        """Get LLM-specific settings as LLMSettings instance."""
        return LLMSettings()
...
def _init_legacy_exports() -> None:
    ...
    global GEMINI_API_KEY, NCBI_EMAIL, NCBI_API_KEY, JINA_API_KEY
    ...
    settings = get_settings()
    GEMINI_API_KEY = settings.effective_google_api_key
